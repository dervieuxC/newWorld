...
{
	$texteRequete="select libelleProd from produit inner join categorie on categorie.idCat=produit.idCat where libelleCat='".$cat."' ;";
	$resultat=mysql_query($texteRequete);
	return $resultat;
}

function listeLots($produit){
    $requete = "Select p.descriptionProd, p.imageProd, l.quantiteLot - l.uniteDeVenteLot as quantiteLot, l.dateRecolteLot, l.nbJourConservationLot, l.modeProductionLot, l.ramassageManuelLot, l.prixUnitaireLot from produit p inner join lot l on p.idProd = l.idProd where p.libelleProd = '".$produit."';";
